export interface GTProjectSection {
  heading: string;
  content: string;
  visual?: GTProjectVisual;
}

export type GTProjectVisual =
  | {
      type: "trading-lab";
      title: string;
    }
  | {
      type: "indicator-console";
      title: string;
    }
  | {
      type: "learner-lab";
...
in sample the strategy returned +15.6% against a benchmark of +1.2%. The manual rules were developed exclusively on in sample data and the  out of sample period was never examined during development.  out of sample the strategy returned -5.9%, compared to the benchmark's -8.4%. The rules didn't generate positive returns in the 2010–2011 period, but they still beat passive holding.`,
      },
      {
        ...
        visual: {
          type: "learner-lab",
          title: "From Market Data to Trading Signal",
        },
...
The BagLearner wraps 20 RTLearners. Each bag trains on a bootstrap sample so each tree sees a different slice of the data. At inference time all 20 trees produce a prediction and the mean of their outputs determines the signal: positive means LONG, negative means SHORT, nearzero means CASH. Averaging across decorrelated trees substantially reduces variance compared to any single tree. The model is fully frozen after add_evidence() returns; testPolicy() only calls query() with no weight updates.`,
      },
      {
        ...
        visual: {
          type: "impact-sweep",
          title: "Market Impact Changes the Policy",
        },
    ...
    sections: [
      {
        ...
        visual: {
          type: "enron-funnel",
          title: "From Corpus to Evidence Candidates",
        },
...
We implemented this using NumPy broadcasting rather than looping over each email. The array of email timestamps and the array of 16 crisis dates are broadcast against each other to compute a full distance matrix in one operation, then we take the row wise minimum to get each email's proximity to the nearest crisis. Anything within 48 hours passes. This ran on the 165,000 Phase 1 emails in seconds and reduced the corpus to about 11,000.`,
      },
      {
        ...
        visual: {
          type: "enron-lexicon",
          title: "Manipulation Signal Console",
        },
...
Initialization sets each component mean to a randomly sampled pixel, computes the initial covariances from those means using compute_sigma(), and sets mixing coefficients to a uniform 1/k. compute_sigma() is implemented as a loop over k components, each using the closed form outer product formula: the covariance for component i is (X - MU[i]).T @ (X - MU[i]) / m.`,
      },
      {
        ...
        visual: {
          type: "em-loop",
          title: "Expectation-Maximization Loop",
        },
...
The new convergence function checks all three parameter sets instead of just the log likelihood. If all values in MU, SIGMA, and PI stay within 10% of their previous iteration values for 10 consecutive iterations, training stops. This catches cases where the likelihood plateaus but individual parameters are still drifting, which the default check would miss.`,
      },
      {
        ...
        visual: {
          type: "em-bic",
          title: "Model Selection and 3D Segmentation",
        },
